...
if( isset($_POST['action']) && $_POST['action'] === "comment" ) {
	...
	$spamCatcher = isset($_POST['url']) ? $_POST['url'] : '';
	
	if(empty($name) || empty($email) || empty($commentText)
		|| empty($articleID) || is_int($articleID) || !empty($spamCatcher) ||
		strstr($commentText, "http://")) {
		$alertMessage = "You did not fill in a required field.";
		$alertType = "error";
	}else {		
		...
		if( $response > 0 ) {
			$alertMessage = "Sucessfully posted new comment.";
			$alertType = "success";		
		}
	}
}
...
<head>
	...
	if(count($blogs) == 1) {
		$title .= $blogs[0]['title'] . " - ";
		$picture = $blogs[0]['picture'];
	}
	$title .= "Trevor's Blog";
	echo "<title>" . $title . "</title>\n\t";
	
	if(count($blogs) == 1) {
		echo "<link rel='image_src' href='http://www.trevor-mack.com" . $picture . "' />\n";
		echo "<meta property='og:image' content='http://www.trevor-mack.com" . $picture . "'/>";
	}
	?>
	...
	<script type=text/javascript>
		$(function() {
			$('div.alert').hide().fadeIn(700);
			$('<span class="exit">X</span>').appendTo('div.alert');
							...
							<div class="blog-body">
								<div id="summary<?php echo $blog['date_posted']; ?>">
									...
									<span style="padding: 0px 10px 5px 0px; float: left;">
										<img style="max-height: 200px; max-width: 200px;" src="<?php echo $blog['picture']; ?>"/>
									</span>
									<?php } else { ?>
							 	...
							 	<div id="blog-comments">
									...
									<?php } ?>
								 	...
								 	<div id="blog-comments-form">
								 		<h3>Add a Comment</h3>
										<form action="<?php echo 'http://trevor-mack.com/blog/'.$year.'/'.$month.'/'.$day ?>" method="post">
											<div class="fname">
											...
											<!-- Deception is the KEY: http://en.wikipedia.org/wiki/Deception-->
											<div class="furl" style="display:none;">
												<label for="url">URL</label>
												<input name="url" type="text" size="100" tabindex="2" />
											</div>
											<div class="fcomment">
